class ChecklistManager {
    ...
    async importTemplate(templateKey) {
        ...
        if (result) {
            // 合并模板到当前清单
            const currentData = this.dataManager.getData('travel_checklist', {
                luggage: [],
                preparation: [],
                important: []
            });
            ...
            // 保存数据
            this.dataManager.setData('travel_checklist', currentData);
    ...
    saveCustomTemplate() {
        ...
        // 保存到本地存储
        const customTemplates = this.dataManager.getData('custom_templates', {});
        const templateKey = 'custom_' + Date.now();
        customTemplates[templateKey] = templateData;
        this.dataManager.setData('custom_templates', customTemplates);
    ...
    // 生成清单分享链接
    generateShareLink() {
        const checklistData = this.dataManager.getData('travel_checklist', {
            luggage: [],
            preparation: [],
            important: []
        });
        ...
        this.dataManager.setData(`share_${shareCode}`, shareData);
    ...
    loadSharedChecklist(shareCode) {
        const shareData = this.dataManager.getData(`share_${shareCode}`, null);
        if (!shareData) {
        ...
        this.modalManager.showConfirmModal(
            ...
            if (result) {
                this.dataManager.setData('travel_checklist', shareData.data);
                this.refreshChecklistDisplay();
    ...
    // 加载清单数据
    loadChecklistData() {
        const data = this.dataManager.getData('travel_checklist', {
            luggage: [],
            preparation: [],
            important: []
        });
    ...
    addItem(category, text) {
        ...
        this.dataManager.setData('travel_checklist', this.checklistData);
        this.refreshChecklistDisplay();
