...
class ItineraryManager {
    constructor() {
        ...
        this.weatherManager = window.weatherManager;
    ...
    renderItinerary(container = null) {
            ...
            html += `
                <div class="itinerary-day" data-date="${date}">
                    ...
                    <div class="day-actions">
                        <button class="btn btn-sm btn-info" onclick="itineraryManager.showWeatherSuggestions('${date}')">
                            <i class="fas fa-cloud-sun-rain"></i> 查看天气建议
                        </button>
                    </div>
                </div>
            `;
        ...
        this.showPlanningToolsButtons();
    }

    // 加载天气信息
    async loadWeatherInfo() {
        if (!this.currentTrip || !this.weatherManager) return;
        
        const destination = this.currentTrip.destination;
        const dates = Object.keys(this.getActivitiesGroupedByDate()).sort();
        
        for (const date of dates) {
            const weather = await this.weatherManager.getWeather(destination, date);
            if (weather) {
                this.updateDayWeatherDisplay(date, weather);
            }
        }
        
        // 检查是否有需要提醒的天气情况
        if (this.aiSystem) {
            const weatherAdjustments = await this.aiSystem.suggestWeatherAdjustments(this.currentTrip.id);
            if (weatherAdjustments && weatherAdjustments.length > 0) {
                this.showWeatherAdjustmentAlerts(weatherAdjustments);
            }
        }
    }

    // 更新日期的天气显示
    updateDayWeatherDisplay(date, weather) {
        const dayElement = document.querySelector(`.itinerary-day[data-date="${date}"] .day-header`);
        if (!dayElement) return;
        
        // 移除已存在的天气信息
        const existingWeather = dayElement.querySelector('.day-weather');
        if (existingWeather) {
        ...
        dayElement.insertAdjacentHTML('beforeend', weatherHTML);
    }

    // 显示天气调整提醒
    showWeatherAdjustmentAlerts(adjustments) {
        adjustments.forEach(adjustment => {
            if (adjustment.severity === 'warning') {
                Utils.showMessage(`[${adjustment.date}] ${adjustment.message}`, 'warning');
            } else {
                Utils.showMessage(`[${adjustment.date}] ${adjustment.message}`, 'info');
            }
        });
    }

    // 显示天气智能建议
    async showWeatherSuggestions(date) {
        if (!this.currentTrip || !this.weatherManager) return;
        ...
        if (!weather) {
            Utils.showMessage('无法获取天气信息', 'warning');
            return;
        }
        ...
        dayActivities.forEach(activity => {
            const suggestions = this.weatherManager.getActivitySuggestions(weather, activity.type);
        ...
        // 生成模态框内容
        const modalContent = `
            <div class="weather-suggestions-modal">
                ...
                <div class="suggestions-section">
                    <h4>行程调整建议</h4>
                    ${suggestionsHTML || '<p>当天天气良好，无需特别调整行程</p>'}
                </div>
            </div>
        `;
        
        this.modalManager.showCustomModal('天气智能提醒', modalContent);
    }
    ...
    showLocationMap(location) {
        ...
        // 打开地图导航
        const mapUrl = `https://uri.amap.com/navigation?to=${encodeURIComponent(location)}&mode=driving`;
        window.open(mapUrl, '_blank');
        
        // 显示交通建议
        this.showTransportationSuggestions(location);
    }

    // 显示交通智能建议
    showTransportationSuggestions(location) {
        if (!this.currentTrip || !this.aiSystem) return;
        
        const transportSuggestions = this.aiSystem.suggestTransportation(this.currentTrip.id);
        if (!transportSuggestions) {
            Utils.showMessage('无法生成交通建议', 'warning');
            return;
        }
        
        // 生成交通建议HTML
        let dailySuggestionsHTML = '';
        if (transportSuggestions.dailySuggestions && transportSuggestions.dailySuggestions.length > 0) {
            dailySuggestionsHTML = `
                <div class="daily-transport-suggestions">
                    <h4>每日交通建议</h4>
                    ${transportSuggestions.dailySuggestions.map(suggestion => `
                        <div class="suggestion-item">
                            <h5>${suggestion.date}</h5>
                            <p>${suggestion.suggestion}</p>
                            <p class="estimated-cost">预估费用: ${Utils.formatCurrency(suggestion.estimatedCost)}</p>
                        </div>
                    `).join('')}
                </div>
            `;
        ...
        const modalContent = `
            <div class="transportation-suggestions-modal">
                <div class="transport-header">
                    <h3>交通智能规划</h3>
                    <span class="location">目的地: ${this.currentTrip.destination}</span>
                </div>
                
                <div class="transport-overview">
                    <h4>总体建议</h4>
                    <p>${transportSuggestions.overall}</p>
                </div>
        ...
        this.modalManager.showCustomModal('交通智能规划', modalContent);
    }

    // 显示行程规划工具按钮
    showPlanningToolsButtons() {
        const plannerContainer = document.querySelector('.planner-container');
        if (!plannerContainer) return;
        
        // 检查按钮容器是否已存在
        let actionsContainer = plannerContainer.querySelector('.planner-actions');
        if (!actionsContainer) {
            actionsContainer = document.createElement('div');
            actionsContainer.className = 'planner-actions';
            plannerContainer.insertAdjacentElement('beforeend', actionsContainer);
        }
        
        // 检查交通规划按钮是否已存在
        if (!actionsContainer.querySelector('.transport-planning-btn')) {
            const transportButton = document.createElement('button');
            transportButton.className = 'btn btn-primary transport-planning-btn';
            transportButton.innerHTML = '<i class="fas fa-route"></i> 交通智能规划';
            transportButton.onclick = () => this.showCompleteTransportationPlan();
            actionsContainer.appendChild(transportButton);
        }
        
        // 检查行程优化按钮是否已存在
        if (!actionsContainer.querySelector('.optimize-planning-btn')) {
            const optimizeButton = document.createElement('button');
            optimizeButton.className = 'btn btn-success optimize-planning-btn';
            optimizeButton.innerHTML = '<i class="fas fa-robot"></i> 行程智能优化';
            optimizeButton.onclick = () => this.showItineraryOptimizationModal();
            actionsContainer.appendChild(optimizeButton);
        }
        
        // 检查分享按钮是否已存在
        if (!actionsContainer.querySelector('.share-planning-btn')) {
            const shareButton = document.createElement('button');
            shareButton.className = 'btn btn-info share-planning-btn';
            shareButton.innerHTML = '<i class="fas fa-share-alt"></i> 分享行程';
            shareButton.onclick = () => {
                if (window.socialShareManager) {
                    window.socialShareManager.showShareOptions(this.currentTrip.id);
                }
            };
    ...
    // 显示行程优化模态框
    showItineraryOptimizationModal() {
        if (!this.currentTrip || !this.aiSystem) {
            Utils.showMessage('请先选择一个旅行计划', 'warning');
            return;
        }
        
        const suggestions = this.aiSystem.getItineraryOptimizationSuggestions(this.currentTrip.id);
        ...
        if (suggestions.length > 0) {
            ...
            suggestionsHTML = '<p>您的行程已经非常合理，无需优化。</p>';
        }
        
        const modalContent = `
            <div class="itinerary-optimization-modal">
                <div class="modal-header">
                    <h3>行程智能优化</h3>
                    <span class="trip-name">旅行计划: ${this.currentTrip.name}</span>
                </div>
                ...
                <div class="modal-body">
                    ...
                    <div class="optimization-options">
                        ...
                        <div class="option-group">
                            <label>
                                <input type="checkbox" id="resolveConflicts" checked>
                                解决时间冲突
                            </label>
                        </div>
                        <div class="option-group">
                        ...
                        <div class="option-group">
                            <label>
                                <input type="checkbox" id="balanceActivities" checked>
                                平衡每日活动数量
                            </label>
                        </div>
                    </div>
                ...
                <div class="modal-footer">
                    ...
                    <button class="btn btn-primary" onclick="itineraryManager.optimizeCurrentItinerary()">开始优化</button>
                </div>
            </div>
        `;
        
        this.modalManager.showCustomModal('行程智能优化', modalContent);
    }
    ...
    optimizeCurrentItinerary() {
        ...
        const result = this.aiSystem.optimizeItinerary(this.currentTrip.id);
        if (result) {
            // 重新渲染行程
            this.renderItinerary();
            Utils.showMessage('行程优化完成！', 'success');
        } else {
            Utils.showMessage('行程优化失败', 'error');
        }
    }

    // 显示完整的交通规划
    showCompleteTransportationPlan() {
        if (!this.currentTrip || !this.aiSystem) {
            Utils.showMessage('请先选择一个旅行计划', 'warning');
            return;
        }
        ...
        const transportSuggestions = this.aiSystem.suggestTransportation(this.currentTrip.id);
        if (!transportSuggestions) {
            Utils.showMessage('无法生成交通规划', 'warning');
            return;
        }
        
        // 显示交通规划模态框
        this.showTransportationSuggestions(this.currentTrip.destination);
    }
