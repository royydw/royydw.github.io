...
class DocumentManager {
    ...
    // 加载证件数据
    loadDocuments() {
        return this.storageManager.getData('travel_documents', []);
    }

    // 保存证件数据
    saveDocuments() {
        this.storageManager.setData('travel_documents', this.documents);
    }
    ...
    addDocument(documentData) {
        const newDocument = {
            ...
            photo: documentData.photo || null, // 证件照片
            status: 'active',
    ...
    // 获取上传的照片数据
    getPhotoData(inputId) {
        const input = document.getElementById(inputId);
        if (input && input.dataset.photoData) {
            return input.dataset.photoData;
        }
        return null;
    }
        ...
        const formHTML = `
            <div class="document-form">
                ...
                <form onsubmit="documentManager.processAddDocument(event)" class="modal-form">
                    ...
                    <div class="form-group">
                        <label>证件照片</label>
                        <div class="document-photo-upload">
                            <input type="file" id="docPhoto" accept="image/*" style="display: none;" 
                                   onchange="documentManager.handlePhotoUpload(this, 'docPhotoPreview')">
                            <label for="docPhoto" class="photo-upload-btn">
                                <i class="fas fa-camera"></i>
                                <span>上传证件照片</span>
                            </label>
                            <img id="docPhotoPreview" class="document-photo-preview" style="display: none;" alt="证件照片预览">
                        </div>
                        <small class="form-hint">支持 JPG、PNG 格式，最大 5MB</small>
                    </div>
                    <div class="form-group">
                        <label>备注</label>
    ...
    processAddDocument(event) {
        ...
        const documentData = {
            ...
            photo: this.getPhotoData('docPhoto') // 获取照片数据
        };
            ...
            return `
                <div class="document-card ${status.class}">
                    ...
                    <div class="document-photo-container">
                        <img src="${doc.photo}" alt="${doc.name}" class="document-photo" onclick="documentManager.viewPhoto('${doc.photo}')">
                    </div>
                    ` : ''}
                    <div class="document-details">
    ...
    viewPhoto(photoUrl) {
        const photoHTML = `
            <div class="document-photo-viewer">
                <img src="${photoUrl}" alt="证件照片" class="photo-fullsize">
            </div>
        `;
        this.modalManager.showCustomModal('证件照片', photoHTML);
    }
}
